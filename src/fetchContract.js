    ...
    try {
        ...
        for (fromBlock; fromBlock < toBlock; fromBlock++) {
            ...
            for (let i = 0; i < 1; i++) {
                ...
                const transactionHash = "0xba0e6045ac7221ccf2be560e2baa13bc7e7fd1b39286d7ee0150c6397ca4b0d9";

                
                let transactionReceipt = await web3.eth.getTransactionReceipt(transactionHash);
                let transactionLogs = transactionReceipt.logs;
                console.log("🚀 ~ file: fetchContract.js ~ line 44 ~ mainLoop ~ transactionLogs", transactionLogs)
